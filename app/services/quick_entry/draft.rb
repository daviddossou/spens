...
module QuickEntry
  ...
  Draft = Data.define(
    ...
    :debt_id, :contact_name, :direction, :unresolved, :amount_candidates
  ) do
    ...
    def initialize(kind:, amount: nil, account_name: nil, from_account_name: nil,
                   ...
                   contact_name: nil, direction: nil, unresolved: [], amount_candidates: [])
      super
    end

    # Several numbers could be the amount and nothing settled it (amount is the best guess).
    def amount_ambiguous?
      unresolved.include?(:amount) && amount.present?
    end
